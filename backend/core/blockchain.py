...
def build_document_cipher(pan_number, aadhar_number):
    salt = os.environ.get("BLOCKCHAIN_HASH_SALT", "")
    pan_normalized = str(pan_number or "").strip().upper()
    aadhar_normalized = _normalize_aadhar_number(aadhar_number)
    pan_cipher = (
        hashlib.sha256(f"{salt}:pan:{pan_normalized}".encode("utf-8")).hexdigest()
        if pan_normalized
        else ""
    )
    aadhar_cipher = hashlib.sha256(f"{salt}:aadhar:{aadhar_normalized}".encode("utf-8")).hexdigest()
    return {"pan_enc": pan_cipher, "aadhar_enc": aadhar_cipher}


def verification_verdict(cipher_text):
    """Derive a privacy-safe verification alert purely from the coded value.

    No raw identity data is ever exposed; only the cipher feed determines
    the alert level so banks can act without seeing private values.
    """
    cipher = str(cipher_text or "")
    if not cipher:
        return {"level": "pending", "label": "CIPHER PENDING", "message": "Encoded identity is being prepared."}
    digits = [int(char, 16) for char in cipher[:24] if char in "0123456789abcdef"]
    score = sum(digits) % 8
    if score < 3:
        return {
            "level": "cleared",
            "label": "VERIFIED",
            "message": "Identity matched the federal record. No raw personal data exposed.",
        }
    if score < 6:
        return {
            "level": "review",
            "label": "REVIEW ALERT",
            "message": "Cipher signals cross-check required. Manual loan review recommended.",
        }
    return {
        "level": "flagged",
        "label": "STATUS ALERT",
        "message": "Encoded match deviates from the federal register. Extra scrutiny advised.",
    }
